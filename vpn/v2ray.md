...
# 3.搭建v2ray服务
`目的：为了把mvess流量伪装成https流量`

## 3.1 获取证书

通过这个两个博客可以获取
https://blog.csdn.net/w_monster/article/details/128191273
https://blog.csdn.net/w_monster/article/details/126352529
...
`将证书cp 到 /data/nginx/data/cert 目录`


`创建nginx 配置文件 nginx.conf`
```shell
worker_processes  1;
events {
...
server {
    listen 8883;
    server_name akataotao.asia;  # 改成你的域名
    rewrite ^(.*) https://$server_name$1 permanent;
}

server {
    listen       1127 ssl http2; # 不再推荐使用443端口，可以使用8000-9000之间的端口
    server_name akataotao.asia;
    charset utf-8;
    ...
    ssl_certificate /etc/nginx/cert/fullchain.pem; # 改成你的证书地址
    ssl_certificate_key /etc/nginx/cert/privkey.pem; # 改成证书密钥文件地址
    ...
    location /awesomepath { # 与 V2Ray 配置中的 path 保持一致
        proxy_redirect off;
        proxy_pass http://127.0.0.1:7777; # 假设v2ray的监听地址是7777
...
--net=host \
--name nginx \
-v /data/nginx/data/conf/nginx.conf:/etc/nginx/nginx.conf \
-v /data/nginx/data/logs:/var/log/nginx \
-v /data/nginx/data/cert:/etc/nginx/cert \
nginx
```
# 4.构建mvess协议
`创建配置文件/data/v2ray/conf/config.json
在部署的时候记得将#注释去掉
`
```json
{
  ...
  "inbounds": [{
    ...
    "settings": {
      "clients": [
        {
          "id": "11c2a696-0366-4524-b8f0-9a9c21512b02", #id用户
          "level": 1
        }
      ]
    },
    "streamSettings": {    
        "network": "ws", 
        "wsSettings": {
          "path": "/awesomepath"  #和nginx配置转发路径一致
        }
      },
    "listen": "::"
  }],
...
`启动脚本`
```shell
docker run -d --net=host  --name v2ray --restart=always -v /data/v2ray/conf:/etc/v2ray teddysun/v2ray
```
# 4.ios客户端配置
协议 : Vmess
地址 : 服务器ip地址
端口 : 填nginx配置的tls端口
UUID: config.json里面的inbounds.settings.clients.id
额外id: 0
算法 : auto
传输方式 : websocket （点开里面还有配置路径，路径填streamSettings.wsSettings.path）
TLS: 开启
...
- name: "v2ray"
  ...
  server: 服务器ip
  port: nginx tls 端口
  uuid: 11c2a696-0366-4524-b8f0-9a9c21512b02
  alterId: 0
  cipher: auto
  udp: false
  tls: true
  #skip-cert-verify: true
  #tls-hostname: akataotao.asia
  network: ws
  # ws-path: /awesomepath
  # ws-headers: 
  #   Host: akataotao.asia 
